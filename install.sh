...
' | tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    aptitude -y install build-essential libtool autotools-dev automake pkg-config libssl1.0-dev libevent-dev bsdmainutils libboost-all-dev libminiupnpc-dev
    aptitude -y install sendmail
    aptitude -y install git
    aptitude -y install pwgen -y
    aptitude -y install default-libmysqlclient-dev libpsl-dev libssh2-1-dev libidn2-0-dev libnghttp2-dev
    clear
